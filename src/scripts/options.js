var MIN_TIME_TEXTBOX_VALUE = 0.8;
...
function shadeBackground(a) {
  ...
  a
    ? b.css("display", "block").animate({ opacity: 0.7 })
...
function updatePageModeControls(a, b) {
  ...
  a.find(".mode .mode_test").attr({ disabled: !b });
}
...
function setPageRegexOrSelector(a, b, c) {
  ...
  if (null === c)
    setPageSettings(a, { mode: "text", regex: null, selector: null });
  else {
    var e = "regex" == b,
    ...
    getAllPages(function (b) {
      ...
      for (var d in b) {
        ...
        var f = JSON.stringify({
          ...
          .replace(/>/g, "&gt;");
        c.push(
          "            \x3c!--PageMonitorAdvancedPageData=" + f + "--\x3e\n"
        );
      }
      c.push("</DL><p>");
      a(c.join(""));
...
function importPagesList(a) {
  for (
    var b = RegExp(
      "(<[aA][^<>]+>[^<>]+</[aA]>)(?:\\s*\x3c!--PageMonitorAdvancedPageData=({.*?})--\x3e)?",
      "g"
    ),
    c,
    e = 0;
    (c = b.exec(a, b.lastIndex));
    ...
    c[2] &&
      (g = JSON.parse(
        c[2].replace(/&amp;/g, "&").replace(/&lt;/g, "<").replace(/&gt;/g, ">")
      ));
    f && (addPage($.extend({ url: f, name: d }, g)), e++);
  }
  return e;
}
...
function initializeColorPicker() {
  ...
  $("#badge_color input")
    ...
    .change(function () {
      var a = $(this).val();
      setSetting(SETTINGS.badge_color, [
    ...
    .change(function () {
      ...
      c =
        1 == c
          ...
          : chrome.i18n.getMessage("minutes", "2");
      d.text(c.split(" ")[1]);
      setSetting(SETTINGS.check_interval, a);
    })
    .change();
  b.val(timeAbsoluteToLog(a))
    .change(function () {
...
function initializeSoundSelector() {
  ...
  a.change(function () {
    ...
    b.attr({ disabled: "" == e });
    c.attr({ disabled: /^$|^chrome-extension:/.test(e) });
  });
...
function initializeShowFullPageDiff() {
  $("#show_full_page_diff input:checkbox")
    .change(function () {
      var a = $(this).prop("checked");
...
function initializeImporter() {
  ...
  $("#import_perform", a).click(function () {
    ...
    b
      ? (chrome.i18n.getMessage("import_success_single"),
...
(function () {
  ...
  initializeAdvancedSwitch = function () {
    ...
    $("#options_switch input").click(function () {
      ...
      $(b ? "#basic_options" : "#advanced_options").slideUp(
        "slow",
        function () {
  ...
  $(".rename").live("click", function () {
    ...
    if (!a.is(":hidden")) {
      var b = $('<input type="text" value="' + a.text() + '" />'),
        ...
        e = $(
          '<input type="button" value="' + chrome.i18n.getMessage("ok") + '" />'
        );
      ...
      e.click(function () {
        a.text(b.val());
        setPageSettings(findUrl(this), { name: b.val() });
        c.click();
      });
    }
  });
...
function initializePageAdvancedToggler() {
  ...
  $(".advanced_controls input[type=checkbox]").live("click", function () {
    ...
    $("input,select", $(this).parent()).not(this).attr({ disabled: !a });
  });
}
function initializePageCheckInterval() {
  ...
  $(".page_interval input[type=range]")
...
function initializePageModeSelector() {
  ...
  $(".mode select").live("change", function () {
    ...
    $(".mode_string", a).keyup();
    $(".mode_pick", a).attr({ disabled: "regex" == $(this).val() });
  });
  $(".mode_string")
    .live("keyup", function () {
...
function initializePageModeTester() {
  ...
  $(".mode_test").live("click", function () {
    var b = findUrl(this),
      ...
      d = $(".mode_string", c).val(),
      f = $(this);
    f.val(chrome.i18n.getMessage("test_progress"))
      .add($(".mode_string", c))
      .attr({ disabled: !0 });
    $.ajax({
      url: b,
      ...
      success: function (c) {
        ("regex" == e
          ? findAndFormatRegexMatches
          : findAndFormatSelectorMatches)(c, d, function (f) {
            $("textarea", a).val(f);
            shadeBackground(!0);
            a.fadeIn();
            cleanAndHashPage(c, e, d, d, function (a) {
              setPageSettings(b, { crc: a });
            });
          });
      },
      error: function () {
      ...
      complete: function () {
        f.val(chrome.i18n.getMessage("test_button"))
          .add($(".mode_string", c))
          .attr({ disabled: !1 });
      },
    });
  ...
  if ("date added" != b) {
    ...
    a.sort(function (a, d) {
      if ("name" == b) (a = a.name), (d = d.name);
        ...
        (a = a.check_interval || c), (d = d.check_interval || c);
      else if ("last check" == b) (a = -a.last_check), (d = -d.last_check);
      else if ("last change" == b)
        (a = -a.last_changed || 0), (d = -d.last_changed || 0);
...
function addPageToTable(a) {
  ...
  b.find(".page_link").attr({ href: a.url, target: "_blank" }).text(d);
  b.find(".favicon").attr({ src: getFavicon(a.url) });
  var f = b.find(".last_check_time");
  f.bind("time_updated", function () {
    var b = $(this);
    getPage(a.url, function (a) {
      var c = a.last_check
        ? describeTimeSince(a.last_check)
  ...
  a.check_interval &&
    (g.children("span").addClass("enabled").removeClass("disabled"),
      $("input", g).attr({ disabled: !1 }),
      $("input[type=checkbox]", g).attr({ checked: !0 }),
      (c = !0));
  ...
  "text" != a.mode &&
    ((c = $(".mode", b)),
      ...
      $("input,select", c).attr({ disabled: !1 }),
      $("input[type=checkbox]", c).attr({ checked: !0 }),
      $("select", c).val(a.mode).change(),
      $(".mode_string", c).val(e).keyup(),
      (c = !0));
  c &&
    ($(".advanced_toggle", b).addClass("toggled"),
      $(".advanced_toggle input", b).attr({ checked: !0 }),
      $(".advanced_controls", b).css({ display: "block" }));
  b.appendTo("#pages");
}
function selectorServer(a, b, c) {
  a &&
    a.selector &&
    ((b = findPageRecord(a.url)),
      0 < b.length &&
      0 < $(".advanced_toggle.toggled", b).length &&
      0 < $(".mode :checked", b).length &&
      ($(".mode select", b).val("selector"),
