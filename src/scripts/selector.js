...
function generateControls() {
  var a =
    '<div id="' +
    FRAME_ID +
    '">   <span title="Pick" class="' +
    ACTIVE_CLASS +
    '">%pick%</span>   <span title="Parent" class="' +
    DISABLED_CLASS +
    '">%parent%</span>   <input type="button" title="Done" value="%done%" disabled="disabled" />   <input type="button" title="Help" value="%help%" /> </div>',
...
function currentElementChanged() {
  ...
  current_element
    ...
    : (parent_button.addClass(DISABLED_CLASS), (current_selector = ""));
}
function elementToSelector(a) {
  ...
  for (; !a.is("body") && !a.attr("id");) {
    var b = a.get(0).tagName.toLowerCase(),
      d = a.get(0).className;
    b = (d = d
      .replace(/chrome_page_monitor_\w+/g, "")
      .replace(/^\s+|\s+$/g, "")
      .replace(/\s+/g, "."))
      ? b + "." + d
      : b;
  ...
  $("body").mousemove(function (a) {
    pick_mode &&
      ($("*").removeClass(TEMP_OUTLINE_CLASS),
        $(a.target).addClass(TEMP_OUTLINE_CLASS));
  });
  ...
  parent_button.click(function () {
    if (!$(this).hasClass(DISABLED_CLASS) && current_element) {
      ...
      a.is("body")
        ...
        : ((current_element = a.get(0)), currentElementChanged());
    }
  });
