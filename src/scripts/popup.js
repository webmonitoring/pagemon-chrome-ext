  ...
  setPageSettings(a, { updated: !1 }, function () {
    ...
    $(b)
      ...
      .slideUp("slow", function () {
        1 == $("#notifications .notification").length
          ? $("#notifications").animate(
            { height: "2.7em", opacity: 1 },
            "slow",
            fillNotifications
          )
          : fillNotifications();
    ...
    await addPage({ url: a.url, name: a.title }, function () {
      ...
      if (b)
        for (var c = 0; c < b.length; c++)
          b[c].fillPagesList && b[c].fillPagesList();
    });
  });
}
  ...
  getAllUpdatedPages(function (b) {
    ...
    0 < b.length
      ? $.each(b, function (a, b) {
        ...
        c.find(".favicon").attr({ src: getFavicon(b.url) });
        c.find(".view_diff").attr({ href: "diff.htm#" + btoa(b.url) });
      ...
      b ||
        (c
          ...
          : ($("#monitor_page").unbind("click").addClass("inactive"),
            ...
            (b = b ? "page_monitored" : "monitor"),
            $("#monitor_page span").text(chrome.i18n.getMessage(b))));
    });
  });
  ...
  getAllPageURLs(function (a) {
    ...
    0 >= a.length - b ||
      ($("#check_now").unbind("click"),
        (a = 0 < b ? { height: "2.7em", opacity: 0.01 } : { opacity: 0.01 }),
        ...
        check(!0, function () {
          $("#notifications").animate({ opacity: 0 }, 400, function () {
            ...
            fillNotifications(function () {
              ...
              a.animate({ height: b + "px" }, "slow", function () {
                a.css("height", "auto").html(c).animate({ opacity: 1 }, 400);
  ...
  chrome.tabs.query({ url: b }, function (a) {
    a.length
      ? ((a = a[0].id),
        ...
        chrome.tabs.update(a, { selected: !0 }))
      : chrome.tabs.create({ url: b, selected: !1 });
  });
  a.preventDefault();
}
...
function setUpHandlers() {
  ...
  $(".page_link,.mark_visited,.view_diff,.stop_monitoring").live(
    "click",
    markPageVisited
  );
